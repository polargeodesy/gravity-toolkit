...
def download_GIA_model():
    ...
    # download GIA model
    HOST = [
        'https://www.atmosp.physics.utoronto.ca',
        '~peltier',
        'datasets',
        'Ice6G_C_VM5a',
        'ICE-6G_High_Res_Stokes_trend.txt.gz',
    ]
    # attempt to download the GIA file
    try:
        fid = gravtk.utilities.from_http(HOST, verbose=True)
    except gravtk.utilities.urllib2.HTTPError as exc:
        pytest.xfail(exc.reason)
    except gravtk.utilities.urllib2.URLError as exc:
        pytest.xfail('Connection Refused')
...
def test_GIA_model_drift_estimate():
    ...
    epoch = 2003.3
    # read GIA model
    GIA_Ylms_rate = gravtk.read_GIA_model(
        GIA_FILE, GIA=GIA, LMAX=LMAX, MMAX=MMAX
    )
    ...
    Ylms = (
        gravtk.gia(lmax=LMAX)
        .from_GIA(GIA_FILE, GIA=GIA, mmax=MMAX)
        .drift(tdec, epoch=epoch)
    )
