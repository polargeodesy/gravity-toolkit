...
def test_julian(YEAR, MONTH):
    ...
    YY, MM, DD, HH, MN, SS = gravtk.time.convert_julian(
        JD, format='tuple', astype=np.float64
    )
    # assert dates
...
def test_decimal_dates(YEAR, MONTH):
    ...
    tdec = gravtk.time.convert_calendar_decimal(
        YEAR, MONTH, day=DAY, hour=HOUR, minute=MINUTE, second=SECOND
    )
...
def test_unix_time():
    ...
    output_time = gravtk.time.convert_delta_time(
        UNIX, epoch1=gravtk.time._unix_epoch, epoch2=(2018, 1, 1), scale=1.0
    )
