...
def test_sea_level():
    ...
    landsea = gravtk.spatial().from_ascii(
        LANDMASK,
        date=False,
        spacing=[1.0, 1.0],
        nlat=180,
        nlon=360,
        extent=[0.5, 359.5, -89.5, 89.5],
        compression='gzip',
    )
    ...
    validation = gravtk.spatial().from_ascii(
        valid_file,
        date=False,
        spacing=[1.0, 1.0],
        nlat=180,
...
def test_harmonics():
    ...
    YlmF = gravtk.gen_harmonics(
        ocean_function, landsea.lon, landsea.lat, LMAX=LMAX, METHOD='fourier'
    )
