...
^^^^^^^^^^^^^^^^^^^^^^
We use `GitHub Actions <https://github.com/polargeodesy/gravity-toolkit/actions>`_ continuous integration (CI) services to build and test the project on Linux (``ubuntu-latest``), Mac (``macos-latest``) and Windows (``windows-latest``) Operating Systems.
The configuration files for this service are in the `GitHub workflows <https://github.com/polargeodesy/gravity-toolkit/tree/main/.github/workflows>`_ directory.
